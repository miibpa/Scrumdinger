...
class StandupDetailModel: ObservableObject {
    ...
    func alertButtonTapped(_ action: AlertAction) {
        ...
        case .openSettings:
            Task { await self.openURL(URL(string: UIApplication.openSettingsURLString)!) }
        }
    }
    ...
    func startMeetingButtonTapped() {
        switch (authorizationStatus() == .denied) {
        case true:
            self.destination = .alert(.speechNoPermission)
        case false:
...
extension AlertState where Action == StandupDetailModel.AlertAction {
    ...
    static let speechNoPermission = AlertState(
        title: TextState(""),
        message: TextState("You don't have permission for speech recognition"),
        buttons: [
            .default(TextState("Open Settings"), action: .send(.openSettings)),
            .cancel(TextState("Nevermind"))
        ]
    )
}
